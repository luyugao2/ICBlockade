proc ::ICBK::initializeDimensions {args} {
...
proc ::ICBK::boundingbox {} {

	foreach el {ax ay az bx by bz cx cy cz ox oy oz} { variable $el }
	# set [set el] [set ::ICBK::$el]; # OMG, this also works!
...
proc ::ICBK::readmaterials {} {

	set ::ICBK::allmaterials {}; # initialize list of materials
	set file [file join $::env(ICBKDIR) database membrane.dat]
	...
	if {![catch {open $file "r"} ch]} {
		...
		if {[llength $result]>0} { set ::ICBK::allmaterials $result; }
		close $ch
	}
}

proc ::ICBK::readbdparticles {} {
	set ::ICBK::listofbdparticles ""; # initialize list of bd particles

	set file [file join $::env(ICBKDIR) database ions.dat]
	set results ""
	if {![catch {open $file "r"} ch]} {
		while {[gets $ch line] >= 0} {
			if {[regexp "^#" $line]} { continue } ;# skip comments
		...
		if {$len>0} {
			set ::ICBK::listofbdparticles $result;
			set ::ICBK::numberofbdparticles [lrepeat $len 0]
		}
		close $ch
	}
}
...
proc ::ICBK::readshapes {} {

	set ::ICBK::allshapes {}   ; # initialize list of shapes
	set ::ICBK::shapeImages {} ;

	set directory [file join $::env(ICBKDIR) database shapes]
	...
	foreach file $files {
		...
		if {![catch {open $file "r"} ch]} {
			...
			if {[llength $result]>0} {
				lappend ::ICBK::allshapes $result
				lappend ::ICBK::shapeImages "[file rootname $file].gif"
			}
			close $ch
		}
	}
...
proc ::ICBK::findPotential {args} {
	if {[llength $args] != 2} { return "" }
	set code1 [lindex $args 0]
	set code2 [lindex $args 1]

    set file [file join $::env(ICBKDIR) database interactions.dat]
    if {![catch {open $file r} in]} {
        ...
        while {[gets $in line] >= 0} {
            ...
            set output [file normalize [file join $::env(ICBKDIR) database [lindex $tok 2]]]
